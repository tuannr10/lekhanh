...
module.exports.run = async function ({ event, api, args, permssion, Users }) {
    ...
    switch (args[0]) {
        case "all": {
            ...
            if (listUser.length == 0) listUser = "Hiện tại chưa có người dùng nào bị cảnh báo";
        ...
        default: {
            if (permssion != 2) {
                ...
                if (!data) return api.sendMessage(`Hiện tại ${name} không có bất cứ lời cảnh báo nào!`, threadID, messageID);
                    ...
                    return api.sendMessage(`Hiện tại ${name} còn lại ${data.warningLeft} lần cảnh báo:\n\n${reason}`, threadID, messageID);
                }
            }
            else {
                try {
                    ...
                    if (data.banned) return api.sendMessage("Tài khoản trên đã bị ban do đã bị cảnh báo 3 lần!", threadID, messageID);
