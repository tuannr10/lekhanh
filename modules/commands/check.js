...
module.exports.run = async ({ args, api, event, Currencies, client, Threads, Users }) => {
   ...
   if (args.length == 0) return api.sendMessage(`===  Bạn có thể dùng  ===\n--------\n- check checkpoint => Lọc thành viên bị bay acc khỏi nhóm\n--------\n- check del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n- check online => Xem thời gian hoạt động bot online \n--------\n- kiem tra tuongtac => Check độ tương tác của bạn\n--------\n- check thread => Lọc nhóm dưới 30 thành viên\n--------\n- check all => Check tất cả độ tương tác các thành viên trong box\n- check user => Lọc cá \n--------\n- check covid => Xem thông tin covid\n--------\n- check luckymayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n- check nude => Check những ảnh gợi cảm\n--------\n=== 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
    var mention = Object.keys(event.mentions);
    const data = await api.getThreadInfo(event.threadID);
    if (args[0] == "checkpoint") {// kick người dùng fb
      if (event.senderID != 100090073282948) return api.sendMessage(`[❗] Chúc bạn may mắn lần sau:))`, event.threadID, event.messageID)
...
const res = await axios.get(`https://vanthuan.name.vn/checknude?link=${encodeURIComponent(linkanh)}`);    
